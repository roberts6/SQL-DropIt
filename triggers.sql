...
VALUES('Jordan', 'Air Jordan Prueba', 10.0, 'Hombre', 500, 6660.00, current_timestamp(), 2);
...
-- trigger que al tener un carrito finalizado completa las tablas 'facturacion', 'warehouse', 'compra', 'acumulado_ventas' y actualiza el stock en 'producto'
DELIMITER $$
...
BEGIN
    ...
    SELECT new.id_carrito, new.finalizado, new.cantidad, new.fk_idProducto, new.fk_idCliente, current_timestamp() INTO v_id_carrito, v_finalizado, v_cantidad, v_id_producto, v_id_cliente, v_fecha;

    SELECT marca, modelo, genero, precio, stock
    INTO v_marca, v_modelo, v_genero, v_precio, v_stock
    FROM producto
    ...
    IF v_id_producto IS NOT NULL THEN
        IF v_finalizado = 1 THEN
            ...
            VALUES (v_id_cliente, v_fecha, v_id_producto, v_modelo, v_cantidad, v_precio, (v_precio * v_cantidad));
            ...
            INSERT INTO acumulado_ventas (producto, id_producto, id_cliente, cantidad, stock_viejo, stock_actual, fecha)
            VALUES (v_modelo, v_id_producto, v_id_cliente, v_cantidad, v_stock, (v_stock - v_cantidad), CURRENT_TIMESTAMP());
